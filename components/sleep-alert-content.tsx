...
function WordReveal({
  ...
  return (
    <div ref={ref} className={className} style={{ ...style, overflow: 'hidden', paddingBottom: '0.15em' }}>
      {words.map((word, i) => (
        <motion.span
          key={i}
...
function TiltCard({
  ...
  const handleMouse = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    // Skip 3D tilt on touch devices — prevents jitter during scroll
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        {[0, 1, 2, 3, 4].map((i) => (
          <motion.div
            key={i}
            className={`absolute rounded-full ${i > 2 ? 'hidden md:block' : ''}`}
            style={{
              width: `min(${200 + i * 180}px, ${60 + i * 25}vw)`,
              height: `min(${200 + i * 180}px, ${60 + i * 25}vw)`,
        ...
        {/* Static ring grid for depth — hide outer rings on mobile */}
        {[0, 1, 2, 3, 4, 5].map((i) => (
          <div
            key={`static-${i}`}
            className={`absolute rounded-full ${i > 3 ? 'hidden md:block' : ''}`}
            style={{
              width: `min(${150 + i * 160}px, ${50 + i * 22}vw)`,
              height: `min(${150 + i * 160}px, ${50 + i * 22}vw)`,
