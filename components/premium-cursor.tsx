...
export default function PremiumCursor() {
  ...
  useEffect(() => {
    // Skip entirely on touch-only devices — no mouse pointer exists
  ...
  return (
    <>
      {/* Small inner dot — follows cursor tightly (hidden on touch via CSS) */}
      <div
        ref={smallRef}
        className="fixed rounded-full pointer-events-none z-[99999] hidden [@media(pointer:fine)]:block"
        style={{
          width: SMALL_DOT,
          height: SMALL_DOT,
          background: '#fff',
      ...
      />
      {/* Larger trailing dot — becomes liquid glass ring on hover (hidden on touch) */}
      <div
        ref={largeRef}
        className="fixed rounded-full pointer-events-none z-[99998] hidden [@media(pointer:fine)]:block"
        style={{
          width: LARGE_DOT,
          height: LARGE_DOT,
          background:
