...
const SECTIONS = [
  {
    heading: 'The Fallacy of Reactive Finance',
    content: `Historically, financial institutions operated on a reactive basis. Credit decisions relied on backward-looking data, fraud detection often flagged transactions only after they had already cleared, and portfolio models struggled to incorporate real-time global events.\n\nToday, AI flips the script from reactive mitigation to proactive prediction. By ingesting vast quantities of unstructured data—from global news feeds and supply chain fluctuations to micro-patterns in consumer transactions—deep learning algorithms can predict market volatility and individual credit risks with unparalleled accuracy.`,
  },
  {
    heading: 'Uncovering Micro-Anomalies in Fraud Detection',
    content: `Fraudsters are sophisticated and constantly evolving their methods, making rule-based systems obsolete. Modern predictive algorithms don’t just look for established patterns of fraud; they learn normal behavior on a granular, individual level.\n\nWhen a transaction deviates even slightly from an individual's unique behavioral fingerprint, an AI model can flag it in milliseconds, all without generating excess false positives that frustrate legitimate customers.`,
  },
  {
    heading: 'Algorithmic Portfolio Management',
    content: `Traditional portfolio management often depends on human analysts digesting market reports and historical trends. Predictive intelligence brings adaptive multi-agent networks to the trading floor. These models can continuously simulate thousands of macroeconomic scenarios, identifying optimal asset allocations well before human analysts have processed the morning news.\n\nThe result is heavily optimized portfolios that are significantly more resilient to market shocks and more capable of exploiting transient market inefficiencies.`,
  },
  {
            ...
            <div className="relative z-10 px-6 md:px-12 lg:px-20 xl:px-32">
              <div className="max-w-[760px] mx-auto">
                ...
                <RevealOnScroll>
                  ...
                  >
                    Artificial intelligence is no longer just optimizing the back-office of broad financial enterprises; it has fundamentally re-engineered the very mechanisms we use to detect fraud, execute trades, and distribute credit.
                  </p>
                </RevealOnScroll>
                ...
                {SECTIONS.map((section, i) => (
                  <div key={section.heading}>
                    ...
                    {/* Pull quotes */}
                    {i === 1 && (
                      <PullQuote text="Machine learning algorithms don't just search for established fraud patterns—they build unique behavioral fingerprints that pinpoint discrepancies in milliseconds." />
                    )}

                    {i === 2 && (
                      <PullQuote text="Equitable predictive intelligence requires true transparency. A financial model must be able to justify its decisions to a human auditor." />
                    )}
                ...
                <RevealOnScroll className="mt-20">
                  <div className="flex flex-wrap gap-3">
                    {['FinTech', 'Predictive AI', 'Fraud Detection', 'Automated Trading', 'Banking', 'Compliance'].map(tag => (
                      <span
                        key={tag}
